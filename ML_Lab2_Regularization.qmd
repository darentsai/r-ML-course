...
Take **LASSO** for example:

1.  Extract columns with non-zero coefficients

```{r}
(lasso.coef <- coef(lasso.cv, s = "lambda.min"))
(non0 <- which(lasso.coef[-1, ] != 0))
```

2.  Refit the model without regularization.

```{r}
x.sub <- x[, non0]
lasso.refit.1 <- glmnet(x = x.sub, y = y, lambda = 0)
```

::: {.callout-important appearance="minimal"}

`lambda = 0` means no shrinkage!

:::

We can also recreate a formula and pass it to the corresponding model (`lm`, `glm`, etc.), i.e.

```{r}
lasso.refit.2 <- lm(reformulate(names(non0), "lpsa"), data = training)
```

where `reformulate(names(non0), "lpsa")` gives

```{r echo = FALSE}
reformulate(names(non0), "lpsa")
```

- Comparison

```{r}
cbind("glmnet(lambda=0)" = coef(lasso.refit.1)[, 1],
      "lm" = coef(lasso.refit.2))
```
...
pred.lasso.refit <- predict(lasso.refit.1, newx = z[, non0])
