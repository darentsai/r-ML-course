...
We can recreate a formula and pass it to the corresponding model (`lm`, `glm`, etc.), i.e.

```{r}
lasso.refit <- lm(reformulate(names(non0), "lpsa"), data = training)
```
...
pred.lasso.refit <- predict(lasso.refit, newdata = testing)
